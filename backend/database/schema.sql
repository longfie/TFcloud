...
CREATE TABLE IF NOT EXISTS `TF_sign_tasks` (
  ...
  `account_id` BIGINT UNSIGNED NOT NULL,
  `plugin_code` VARCHAR(64) CHARACTER SET ascii COLLATE ascii_bin NOT NULL,
  `queue_name` VARCHAR(64) CHARACTER SET ascii COLLATE ascii_bin NOT NULL DEFAULT 'default',
  ...
  KEY `idx_queue_dispatch` (`queue_name`, `status`, `available_at`, `priority`, `id`),
...
CREATE TABLE IF NOT EXISTS `TF_bilibili_live_sessions` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `session_no` CHAR(32) CHARACTER SET ascii COLLATE ascii_bin NOT NULL,
  `task_id` BIGINT UNSIGNED NOT NULL,
  `run_id` BIGINT UNSIGNED NOT NULL,
  `user_id` BIGINT UNSIGNED NOT NULL,
  `account_id` BIGINT UNSIGNED NOT NULL,
  `status` VARCHAR(32) CHARACTER SET ascii COLLATE ascii_bin NOT NULL DEFAULT 'discovering',
  `target_heartbeat_count` SMALLINT UNSIGNED NOT NULL DEFAULT 70,
  `max_failures` SMALLINT UNSIGNED NOT NULL DEFAULT 5,
  `max_rooms` SMALLINT UNSIGNED NOT NULL DEFAULT 5,
  `settings_json` JSON DEFAULT NULL,
  `summary_json` JSON DEFAULT NULL,
  `next_action_at` DATETIME(3) DEFAULT NULL,
  `lock_token` CHAR(32) CHARACTER SET ascii COLLATE ascii_bin DEFAULT NULL,
  ...
  `updated_at` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3),
  PRIMARY KEY (`id`),
  UNIQUE KEY `uniq_bilibili_live_session_no` (`session_no`),
  KEY `idx_bilibili_live_task` (`task_id`),
  KEY `idx_bilibili_live_account_status` (`account_id`, `status`, `created_at`),
  KEY `idx_bilibili_live_expiry` (`status`, `expires_at`),
  KEY `idx_bilibili_live_due` (`status`, `next_action_at`, `lock_until`, `id`),
  CONSTRAINT `fk_bilibili_live_task` FOREIGN KEY (`task_id`) REFERENCES `TF_sign_tasks` (`id`) ON DELETE CASCADE,
  CONSTRAINT `fk_bilibili_live_user` FOREIGN KEY (`user_id`) REFERENCES `TF_users` (`id`) ON DELETE CASCADE,
  CONSTRAINT `fk_bilibili_live_account` FOREIGN KEY (`account_id`) REFERENCES `TF_plugin_accounts` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;

CREATE TABLE IF NOT EXISTS `TF_bilibili_live_room_sessions` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `session_id` BIGINT UNSIGNED NOT NULL,
  `room_id` BIGINT UNSIGNED NOT NULL,
  `ruid` BIGINT UNSIGNED NOT NULL,
  `medal_target_id` BIGINT UNSIGNED NOT NULL DEFAULT 0,
  `parent_area_id` BIGINT UNSIGNED NOT NULL DEFAULT 0,
  `area_id` BIGINT UNSIGNED NOT NULL DEFAULT 0,
  `room_name` VARCHAR(255) DEFAULT NULL,
  `medal_name` VARCHAR(120) DEFAULT NULL,
  `medal_level` SMALLINT UNSIGNED NOT NULL DEFAULT 0,
  `status` VARCHAR(32) CHARACTER SET ascii COLLATE ascii_bin NOT NULL DEFAULT 'entering',
  `sequence_no` SMALLINT UNSIGNED NOT NULL DEFAULT 0,
  `heartbeat_count` SMALLINT UNSIGNED NOT NULL DEFAULT 0,
  `consecutive_failures` SMALLINT UNSIGNED NOT NULL DEFAULT 0,
  `server_timestamp` BIGINT UNSIGNED DEFAULT NULL,
  `secret_key` VARCHAR(512) DEFAULT NULL,
  `secret_rule_json` JSON DEFAULT NULL,
  `device_uuid` CHAR(36) CHARACTER SET ascii COLLATE ascii_bin DEFAULT NULL,
  `next_heartbeat_at` DATETIME(3) DEFAULT NULL,
  ...
  `updated_at` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3),
  PRIMARY KEY (`id`),
  UNIQUE KEY `uniq_bilibili_live_room` (`session_id`, `room_id`),
  KEY `idx_bilibili_room_due` (`status`, `next_heartbeat_at`, `lock_until`, `id`),
  KEY `idx_bilibili_room_session_status` (`session_id`, `status`, `id`),
  CONSTRAINT `fk_bilibili_live_room_session` FOREIGN KEY (`session_id`) REFERENCES `TF_bilibili_live_sessions` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
...
('bilibili', '哔哩哔哩', '0.4.0', 'enabled', NOW(), NOW()),
